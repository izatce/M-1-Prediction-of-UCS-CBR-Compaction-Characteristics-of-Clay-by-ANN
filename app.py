...
with t6:
    st.header("Stage 6: Independent External Unseen Dataset")
    if st.session_state.trained is None:
...
with t7:
    st.header("Stage 7: Explainable AI (SHAP) + Gemini Interpretation")
    if st.session_state.trained is None:
        st.info("Train the ANN model first.")
    else:
        trained = st.session_state.trained
...
st.markdown("---")
st.caption("ANN research workflow: data selection → 80/20 split → 5-fold architecture optimization → final development training → holdout/external testing → diagnostics → SHAP → interpretation")
