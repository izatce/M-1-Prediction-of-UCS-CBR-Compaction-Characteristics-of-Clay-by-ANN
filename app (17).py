...
def generate_candidate_architectures(n_samples, n_features):
    """
    Generate an automatically sized ANN search space.

    Hidden-layer depth is NOT fixed at 1, 2, or 3. The search considers
    1, 2, 3, ... layers up to a conservative data-size-dependent maximum.
    Widths are also generated automatically from sample/feature counts.
    """
    n_samples = max(int(n_samples), 10)
    n_features = max(int(n_features), 1)

    # Practical upper bound. For ~100 samples this allows several depths
    # without creating an unnecessarily huge/deep search.
    max_depth = min(6, max(1, int(np.floor(np.log2(n_samples))) - 1))

    width_cap = min(128, max(8, int(np.sqrt(n_samples) * 8)))
    base_widths = sorted(set([
        8, 16, 32, 64,
        min(128, max(8, 2 * n_features)),
        min(128, max(8, 4 * n_features)),
    ]))

    architectures = []
    for depth in range(1, max_depth + 1):
        for base in base_widths:
            base = min(base, width_cap)
            if base < 4:
                continue

            # Decreasing/pyramidal network.
            pyramid = tuple(max(4, int(round(base / (2 ** i)))
                              ) for i in range(depth))
            if all(pyramid[i] >= pyramid[i + 1]
                   for i in range(len(pyramid) - 1)):
                architectures.append(pyramid)

            # A second taper pattern provides another family at each depth.
            if depth >= 2:
                taper = tuple(max(4, int(round(base / (1.5 ** i)))
                                 ) for i in range(depth))
                if all(taper[i] >= taper[i + 1]
                       for i in range(len(taper) - 1)):
                    architectures.append(taper)
...
st.caption("Open automatic architecture optimization • 80/20 holdout • 5-fold CV • independent unseen testing • training diagnostics • SHAP")

st.sidebar.header("⚙️ ANN Configuration")
st.sidebar.success("🤖 Open Automatic Architecture Search")
st.sidebar.write(
    "The number of hidden layers is NOT fixed. The app automatically searches "
    "from 1 layer upward, with a practical data-size-based limit."
)
...
st.sidebar.markdown(
    "**Validation design:** 80% development/training + 20% untouched holdout testing. "
    "5-fold CV is performed only within the 80% development set for architecture selection."
)
...
with t2:
    ...
    if not ready:
        ...
        preview_architectures = generate_candidate_architectures(
            n_samples=max(10, int(len(X_all) * 0.80)),
            n_features=len(st.session_state.features_widget),
        )
        st.info(
            f"Before training, the app will automatically compare "
            f"**{len(preview_architectures)} architectures** spanning "
            f"**1 to {max(len(a) for a in preview_architectures)} hidden layers**. "
            f"No hidden-layer count is manually fixed."
        )
        with st.expander("🔎 View automatically generated architecture search space"):
            st.dataframe(
                pd.DataFrame({
                    "Architecture": [str(a) for a in preview_architectures],
                    "Hidden Layers": [len(a) for a in preview_architectures],
                    "Total Neurons": [sum(a) for a in preview_architectures],
                }),
        ...
        if st.button("🚀 Automatically Optimize and Train ANN", type="primary"):
            try:
                ...
                st.info(
                    f"Automatic architecture search generated "
                    f"**{len(candidate_architectures)} candidate networks** "
                    f"covering **1 to {max(len(a) for a in candidate_architectures)} "
                    f"hidden layers** for this dataset."
                )
                ...
                for i, arch in enumerate(candidate_architectures):
                    status.write(
                        f"5-fold CV: evaluating architecture {arch} "
                        f"({i + 1}/{len(candidate_architectures)}) ..."
                    )
...
10. **5-fold CV:** automatically generated candidate architectures are compared using RMSE, MAE, R² and MAPE.
